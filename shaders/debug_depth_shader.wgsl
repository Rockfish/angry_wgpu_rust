...
// shadow texture
@group(2) @binding(0) var shadow_map_texture: texture_depth_2d;
...
//    let texCoord = coord.xy * 0.5 + 0.5;  // Transform from [-1, 1] to [0, 1] range
    let depth = textureSample(shadow_map_texture, shadow_map_sampler, in.tex_coords);
//    let depth = result.z;
    return vec4<f32>(depth, depth, depth, 1.0);  // Visualize depth as grayscale
}
